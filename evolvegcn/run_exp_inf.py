...
def build_tasker(args, dataset):
    if args.task == 'link_pred':
        ...
        return nct.Node_Reg_Tasker(args, dataset)
    else:
...
def build_classifier(args,tasker):
    if 'node_cls' == args.task or 'static_node_cls' == args.task or 'node_reg' == args.task:
        mult = 1
    else:
        mult = 2
...
if __name__ == "__main__":
    ...
    print('dataset built')
    #build the tasker
    print("task type", args.task)
    tasker = build_tasker(args, dataset)
    # raise
    #build the splitter
    if args.task == "node_reg":
        splitter = sp.Author_Inf_Splitter(args, tasker)
    else:
